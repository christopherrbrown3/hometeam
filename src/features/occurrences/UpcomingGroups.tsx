...
export function UpcomingGroups({ occurrences, onOpen }: Readonly<{ occurrences: OccurrenceWithTitle[]; onOpen: (item: OccurrenceWithTitle) => void }>) {
  ...
  for (const occurrence of occurrences) { const day = new Date(occurrence.original_due_start).toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric', timeZone: occurrence.householdTimeZone }); groups.set(day, [...(groups.get(day) ?? []), occurrence]) }

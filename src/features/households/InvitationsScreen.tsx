...
export function InvitationsScreen({ householdId }: Readonly<{ householdId: string }>) {
  ...
  async function copyNewInvitation(username: string, role: 'full_member' | 'guest') {
    setError(null)
    const invitation = await createInvitation(supabase, householdId, username, role)
    await navigator.clipboard?.writeText(invitationLink(invitation.token))
    setMessage('A private invitation link was copied. It is valid only for the invited username.')
  ...
  async function invite(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await copyNewInvitation(String(form.get('username')), String(form.get('role')) as 'full_member' | 'guest')
